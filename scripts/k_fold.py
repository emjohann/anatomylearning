...
# Define save folder
save_folder = f'.5_fold/{n_folds}Fold_Results_{select_label}_TH_{threshold}/{current_datetime}/'
...
print("\n" + "="*80)
print("APPLYING Z-SCORE NORMALIZATION (ROW-WISE)")
print("="*80)
...
print("\n" + "="*80)
print(f"{n_folds}-fold started (subject independent)")
print("="*80)

# Shuffle participants and create n_folds
np.random.seed(42)
shuffled_participants = np.random.permutation(participant_ids)
kfold = KFold(n_splits=n_folds, shuffle=False)

for fold, (train_idx, test_idx) in enumerate(kfold.split(shuffled_participants), 1):
    # Get participants for this fold
    train_participants = shuffled_participants[train_idx]
    test_participants = shuffled_participants[test_idx]
    ...
    train_mask = df2['Participant ID'].isin(train_participants)
    test_mask = df2['Participant ID'].isin(test_participants)
    ...
    for model_name, model in models.items():
        ...
        ml_results.append({
            'Model': model_name,
            'Fold': fold,
            'Test_Participants': str(test_participants.tolist()),
            'Accuracy': acc,
            'Precision': prec,
            'Recall': rec,
            'F1_Macro': f1_macro
...
mean_row = pd.DataFrame({
    'Fold': ['Mean'], 'Test_Participants': ['-'], 'Best_Epoch': ['-'],
...
std_row = pd.DataFrame({
    'Fold': ['Std'], 'Test_Participants': ['-'], 'Best_Epoch': ['-'],
